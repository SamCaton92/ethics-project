...
class AnimalLookupApp:
    ...
    def search_animals(self):
        """Fetch animal data from Galah based on filters, including coordinates, and display the results."""
        try:
            ...
            # Fetch data from Galah
            atlas_output = galah.atlas_occurrences(
                filters=filters,
                bbox=bbox,
                fields=["vernacularName", "scientificName", "decimalLatitude", "decimalLongitude"]
            )
            print("Atlas output preview:", atlas_output.head())  # For debugging

            # Check that required columns exist and populate the Listbox
            if "vernacularName" in atlas_output.columns and "decimalLatitude" in atlas_output.columns:
                # Populate dictionary with display names
                self.animal_data = atlas_output[
                    ["vernacularName", "scientificName", "decimalLatitude", "decimalLongitude"]
                ]

                # Clear Listbox and insert animal names
                self.animal_listbox.delete(0, tk.END)
                for _, row in self.animal_data.iterrows():
                    name_with_coords = f"{row['vernacularName']} (Lat: {row['decimalLatitude']}, Lon: {row['decimalLongitude']})"
                    self.animal_listbox.insert(tk.END, name_with_coords)
            else:
